...
> 수행 내용
1. 데이터 다운로드 및 로드(아스키코드 값 제거)
2. 데이터 전처리(중복 제거, Null값 제거, 영어와 공백, ' 를 제외한 문자 제거)
   -> 전처리까지 수행한 결과 저장(train_data_set.csv / dev_data_set.csv / test_data_set.csv)
3. 데이터 정제(축약문 처리, 반복 문자열 교정)
   -> 정제까지 수행한 결과 저장(train_last_data.csv / dev_data_set.csv / test_last_data.csv)
4. 토큰화
5. 불용어 제거
6. 표제어 추출(Lemmatization) / 어간 추출(Stemming)
7. 정수 인코딩
8. 패딩
9. RNN-LSTM 모델 설정 및 학습/평가(모델 학습시, 학습 데이터의 20%를 검증용으로 사용)
...
> 실행 방법
'[영어 감정분석기]_모델 실행 및 평가' 코드의 en_data.csv 부분에 평가할 새로운 데이터(Private Data) 파일명 명시(아래 코드 참조)
----------------------------------------------------------------------------
